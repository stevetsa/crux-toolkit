...
int MatchSearch::main(int argc, char** argv){
  ...
  initializeRun(argument_list, num_arguments, option_list, 
...
string MatchSearch::getFileString() {
  return "search";
}
 ...
 */
bool MatchSearch::is_search_complete(MATCH_COLLECTION_T* matches, 
 ...
 */
int MatchSearch::search_pep_mods(
  ...
  for(mod_idx=0; mod_idx<num_peptide_mods; mod_idx++){
    ...
    if( this_aa_mods > cur_aa_mods ){
      ...
      bool passes = is_search_complete(match_collection, cur_aa_mods);
      if( passes ){
 ...
 */
void MatchSearch::print_spectrum_matches(
 ...
 */
void MatchSearch::add_decoy_scores(
