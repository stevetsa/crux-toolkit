...
bool ModifiedPeptidesIterator::queueNextPeptide(){
  ...
  if( ! is_empty_linked_list( temp_peptide_list_)){
    ...
    next_peptide_ = (PEPTIDE_T*)pop_front_linked_list(temp_peptide_list_);
  ...
  PEPTIDE_T* unmod_peptide = peptide_source_->next();
  ...
  IF_CARP_DETAILED_DEBUG(
    char* seq = get_peptide_modified_sequence_with_masses(next_peptide_, FALSE);
