...
bool Ms2SpectrumCollection::parse() {
  ...
  Spectrum* parsed_spectrum = Spectrum::newSpectrumFromFile(file, 
                                                            filename_.c_str());
  while(parsed_spectrum){
    // include this a scan?
    if( parsed_spectrum->getFirstScan() < first_scan ){
      delete parsed_spectrum;
      parsed_spectrum = Spectrum::newSpectrumFromFile(file, filename_.c_str());
      continue;
    } 
  ...
  )
{
  ...
  if(!spectrum->parseFile(file, filename_.c_str())){
    fclose(file);
    return false;
  }
  fclose(file);
  ...
  )
{
  ...
  fseek(file, target_index, SEEK_SET);
  // parse spectrum, check if failed to parse spectrum return false
  return_spec = Spectrum::newSpectrumFromFile(file, filename_.c_str());
  fclose(file);
  return return_spec;
}
